...
def writeFrame(img):
    ...
    global videonumber
    output_name = "./output/outputvideo_{}.mp4".format(videonumber)
     
    if writer_frame_count >= 120 :
        writer = None
        videonumber += 1
        writer_frame_count = 0
       
    if writer is None and output_name is not None :
        writer = cv2.VideoWriter(output_name, fourcc,30, (img.shape[1], img.shape[0]),True)
...
while True:
        ...
        if writer_flag == 1:
            cv2.imshow("real",cp_frame)
            cv2.putText(frame, str(writer_frame_count) + ' frame', (10, height - ((i * 20) + 50)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
            if writer_frame_count <= 120 and writer_frame_count >= 30:
                writeFrame(cp_frame)
